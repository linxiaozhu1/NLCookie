	...
	core.AddCommand("jd", []core.Function{
			...
			Handle: func(s core.Sender) interface{} {
				...
				if s.GetImType() == "wxmp" && !ke {
					if len(cks) <= 2 {
						...
						return "您拥有多个账号，输入任意字符将依次为您展示查询结果："
					}
		...
		},
		{
			...
			Handle: func(s core.Sender) interface{} {
				...
				s.Reply(fmt.Sprintf("已为你找到%d条结果，请在60秒内回复“查看”，将依次为你展示~~~", len(cookies)))
				var ids = []string{}
				for i := range cookies {
						...
						if l != 0 {
							...
							for {
								...
								s.Await(s, func(_ core.Sender) interface{} {
									...
									return fmt.Sprintf("闲的蛋疼吗？查查查，查个屁查~~~\n🐶京东账号%d秒内废掉了，请稍后再进行查询~~~", left)
