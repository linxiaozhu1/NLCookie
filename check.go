	...
	core.AddCommand("jd", []core.Function{
		{
			...
			Handle: func(s core.Sender) interface{} {
				...
				for pin, env := range cks {
					...
					if ck.Available() {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE有效~~~", ck.Nickname), core.E)
						delete(wscks, pin)
						continue
					}
					s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE已失效~~~", pin), core.E, core.N)
					Notify(pin, fmt.Sprintf("您的🐶京东账号：%s 已过期~~~\n请重新私聊我发送命令：登录，进行登录~~~\n或添加管理员微信：Lin-VowNight，进行人工登录~~~\n\n本通知连续出现二次以上才是真正的帐号失效!!!", pin))
					if _, err := qinglong.Req(qls[0], qinglong.PUT, qinglong.ENVS, "/disable", []byte(`["`+env.ID+`"]`)); err != nil {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE禁用失败~~~%v", pin, err), core.E)
					} else {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE已禁用~~~", pin), core.E)
					}
					wse, ok := wscks[pin]
					if !ok {
						continue
					...
					if err != nil {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_WSCK转换失败~~~%v", pin, err), core.E, core.N)
						delete(wscks, pin)
						continue
					}
					if strings.Contains(pt_key, "fake") {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_WSCK已失效~~~", pin), core.E)
						if jdWSCK.GetBool("autoDisableWsck", false) {
							if _, err := qinglong.Req(qls[0], qinglong.PUT, qinglong.ENVS, "/disable", []byte(`["`+wse.ID+`"]`)); err != nil {
								s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_WSCK禁用失败~~~%v", pin, err), core.E)
							} else {
								s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_WSCK已禁用~~~", pin), core.E)
							}
						}
						delete(wscks, pin)
						continue
					}
					s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_WSCK转换JD_COOKIE成功~~~", pin), core.E, core.N)
					if _, err := qinglong.Req(qls[0], qinglong.PUT, qinglong.ENVS, "/enable", []byte(`["`+env.ID+`"]`)); err != nil {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE启用失败~~~%v", pin, err), core.E)
					} else {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE已启用~~~", pin), core.E)
					}
					env.Status = 0
					env.Value = fmt.Sprintf("pt_key=%s;pt_pin=%s;", pt_key, pin)
					if err := qinglong.UdpEnv(qls[0], env); err != nil {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE更新失败~~~%v", pin, err), core.E)
					} else {
						s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE已更新~~~", pin), core.E)
					}
					delete(wscks, pin)
				}
				for pin, wse := range wscks {
					...
					if env, ok := cks[pin]; ok {
						...
						if err := qinglong.AddEnv(qls[0], qinglong.Env{
							...
							s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE添加失败~~~%v", pin, err), core.E)
						} else {
							s.Reply(fmt.Sprintf("🐶京东账号：%s，JD_COOKIE已添加~~~", pin), core.E)
						}
					}
				}
				s.Reply("🐶京东账号检查完成，又可以愉快的玩耍啦~~~", core.E)
				return nil
			},
		},
	})
