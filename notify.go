...
func assetPush(pt_pin string) {
	...
	if jn.PushPlus != "" {
		...
		if days < 1000 {
			...
			if days > 25 {
				head += "\n⚠️⚠️⚠️您的🐶京东账号即将过期，请重新私聊我发送命令：登录，进行登录~~~\n或添加管理员微信：Lin-VowNight，进行人工登录~~~\n\n"
			} else {
				head += "\n\n"
			}
		}
...
func a叉哦叉哦(pt_pin, class, content string) {
	...
	if u.PushPlus != "" {
		pushpluspush(class+"通知", content+"\n\n通知太烦人？可发送命令：“关闭"+class+"通知”或“账号管理”，根据菜单自定义设置你的账号通知信息~~~", u.PushPlus)
		return
	}
	Notify(pt_pin, class+"通知("+u.Note+")：\n"+content+"\n\n通知太烦人？可发送命令：“关闭"+class+"通知”或“账号管理”，根据菜单自定义设置你的账号通知信息~~~")
}
...
func (ck *JdCookie) QueryAsset() string {
	...
	if ck.Available() {
		...
		msgs = append(msgs, fmt.Sprintf("\n⚠️部分活动若无数据显示&手动开启活动⚠️"))
		msgs = append(msgs, fmt.Sprintf("⚠️请发送命令：入口，获取活动页二维码⚠️"))
		msgs = append(msgs, fmt.Sprintf("⚠️特殊情况部分数据会异常,请稍后查询⚠️"))
		// }
		// if ck.Note != "" {
		// 	msgs = append([]string{
		...
		msgs = append(msgs, []string{
			// "提醒：该🐶京东账号已过期~~~\n请重新私聊我发送命令：登录，进行登录~~~\n\n多🐶京东账号的🐑毛党员注意了，登录第2个🐶京东账号的时候，不可以退出第1个🐶京东账号，退出会造成账号过期。可以在登录第2个🐶京东账号前清除浏览器cookie，或者使用浏览器的无痕模式!!!",
			"提醒：该🐶京东账号已过期~~~\n请重新私聊我发送命令：登录，进行登录~~~\n或添加管理员微信：Lin-VowNight，进行人工登录~~~\n\n本通知连续出现二次以上才是真正的帐号失效!!!",
		}...)
	}
	ck.PtPin, _ = url.QueryUnescape(ck.PtPin)
